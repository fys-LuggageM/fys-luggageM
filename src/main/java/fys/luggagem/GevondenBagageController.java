...
public class GevondenBagageController implements Initializable {
    ...
    @FXML
    private void handleImportFoundLuggageAction2(ActionEvent event) {
        File file = MainApp.selectFileToRead("*.xlsx");
        if (file != null) {
            String fileName = file.getAbsolutePath();
            List<GevondenBagageExcelImport> foundluggage = 
                    GevondenBagageExcelImport.importFoundLuggageFromExcel(fileName);
